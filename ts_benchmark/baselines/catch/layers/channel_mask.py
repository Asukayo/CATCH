...
class channel_mask_generator(torch.nn.Module):
    def __init__(self, input_size, n_vars):
        ...
        self.generator = nn.Sequential(
            ...
            torch.nn.Linear(input_size, n_vars, bias=False), nn.Sigmoid())
    ...
    # bs：batch_size   patch_num:分片数量   n_vars 变量/通道数量  patch_size:每个分片的大小
    def forward(self, x):  # x: [(bs x patch_num) x n_vars x patch_size]

        # 生成概率分布矩阵
        # 输入数据通过生成器网络，得到每个通道的选择概率
        # 输出形状[(bs x patch_num) x n_vars x n_vars]
